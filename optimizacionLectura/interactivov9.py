...
class DoseApp:
    ...
    def save_subcircles_to_file(self):
        ...
        # Crear nombre de archivo
        filename = f"Resultados_Posillos_{date_str}.txt"
        
        try:
            with open(filename, "w", encoding="utf-8") as file:
                # Información general
                file.write("=== RESULTADOS DE ANÁLISIS DE POSILLOS ===\n\n")
                ...
                file.write("Posillo\tDosis (Gy)\tσ (Gy)\tError\n")
                file.write("-" * 50 + "\n")
            ...
            print(f"✅ Resultados de posillos guardados en '{filename}'")
            ...
            print(f"❌ Error al guardar resultados de posillos: {e}")
            if self.subcircles_window and self.subcircles_window.winfo_exists():
    ...
    def create_subcircles_window(self, circle_data, dose_map):
        ...
        self.subcircles_window.title(f"Análisis de Posillos - {circle_data['id']}")
        self.subcircles_window.geometry("900x600")  # Reducido de 900x700 a 800x600
        ...
        centro_x = w // 2
        centro_y = h // 2
        radio_maximo = min(w, h) / 2
        radio_sub = radio_maximo / 4.2
        radio_grande = (radio_maximo - radio_sub)* 1.2
        
        # Dibujar círculo grande
        circle_grande = plt.Circle((centro_x, centro_y), radio_grande, fill=False, linewidth=1)
        ...
        self.subcircles_ax.set_title(f"Análisis de Posillos - {circle_data['id']}")
        ...
        subcircle_info_frame = tk.LabelFrame(info_frame, text="Información de Posillos", padx=5, pady=5)
        ...
        self.subcircle_info_text.insert(tk.END, "Posillo  Dosis (Gy)  σ (Gy)  Error\n")
    ...
    def procesar_circulo(self, img_rgb, x, y, r, step=1, factor_radio=0.9, graficar=False):
        ...
        if graficar and len(valores_validos) > 0:
            ...
            # Añadir botón para mostrar análisis de posillos
            button_ax = fig.add_axes([0.4, 0.05, 0.2, 0.05])
            button = plt.Button(button_ax, 'Dosis Posillos', color='lightgoldenrodyellow', hovercolor='0.975')
