...
class DoseApp:
    def __init__(self, root):
        ...
        styled_button(main_buttons_frame, "Cargar Imagen", self.load_image).pack(pady=4, fill="x")
        styled_button(main_buttons_frame, "Detect Radiocromicas", self.detectar_areas_radiocromicas).pack(pady=4, fill="x")
        styled_button(main_buttons_frame, "Nombrar Radiocromicas", self.mostrar_dialogo_nombres).pack(pady=4, fill="x")
        styled_button(main_buttons_frame, "Detectar círculos", self.detectar_circulos_y_calcular_dosis).pack(pady=4, fill="x")
        styled_button(main_buttons_frame, "Mapa 3D de dosis", self.generate_dose_map_3d).pack(pady=4, fill="x")
        ...
        styled_button(save_frame, "Guardar medición", self.save_measurement).pack(pady=4, fill="x")
        styled_button(save_frame, "Ver lista de dosis", self.show_dose_list).pack(pady=4, fill="x")
        ...
        styled_button(tools_panel, "Añadir círculo manual", self.add_manual_circle).pack(pady=4, fill="x")
        styled_button(tools_panel, "Eliminar último círculo manual", self.remove_last_manual_circle).pack(pady=4, fill="x")
        ...
        try:
            with open(filename, "w", encoding="utf-8") as file:
                ...
                for area in self.radiochromic_areas:
                    ...
                    # Obtener el fondo del área (promedio de círculos manuales)
    ...
    def get_area_background(self, area):
        """Obtiene el fondo de un área como el promedio de los círculos manuales"""
        manual_circles = [circle for circle in area["circles"] if "manual" in circle and circle["manual"]]
        if manual_circles:
            return np.mean([circle["mean_dose"] for circle in manual_circles])
        return self.background_var.get()  # Si no hay círculos manuales, usar el fondo medido
    ...
    def load_image(self):
        ...
        # Configurar la región de desplazamiento para incluir toda la imagen
        self.canvas.config(scrollregion=(0, 0, self.pil_img.width, self.pil_img.height))

        self.canvas.bind("<Button-1>", self.on_click)
        
        # Limpiar estadísticas y áreas
    ...
    def procesar_circulo(self, img_rgb, x, y, r, step=1, factor_radio=0.9, graficar=False):
        ...
        if graficar and len(valores_validos) > 0:
            ...
            info_text = (
                ...
                f"Error: {(std_dose/mean_dose*100):.2f}%\n"
